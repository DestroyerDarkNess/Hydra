...
using HydraEngine.Protection.Mutations.Blocks;
using HydraEngine.Protection.Mutations.Emulator;
...
{
    public class MutationProt : Models.Protection
    {
        public MutationProt() : base("Protection.Mutations.MutationProt", "Renamer Phase", "Description for Renamer Phase") { }
        ...
        public void Mutate(ModuleDef module, MethodDef method)
        {
            if (!method.HasBody)
                return;
        ...
        public void SimpleUpdateGen(Block block, Local local, int caseValue, int quantity = 1)
        {
        ...
        public int Calculate(int a, int b, out OpCode reverse)
        {
            reverse = OpCodes.Nop;
